...
/**
 * Regra de Arredondamento Acadêmico Oficial do Colégio IMPACTO (ConfigArredondamento):
 * Tabela de Frações (Imagem do Esquema de Notas):
 * Código 1: >= 0,00 e <= 0,25 -> 0,00
 * Código 2: >  0,25 e <= 0,75 -> 0,50
 * Código 3: >  0,75 e <= 1,00 -> 1,00
 *
 * Exemplos:
 * 7.12 -> 7.0 | 7.25 -> 7.0 | 7.26 -> 7.5 | 7.75 -> 7.5 | 7.76 -> 8.0 | 9.80 -> 10.0
 */
export function arredondarMediaImpacto(valor: number): number {
  if (isNaN(valor) || valor <= 0) return 0
  if (valor >= 10) return 10

  const intPart = Math.floor(valor)
  // Fix precisão ponto flutuante: arredonda fração para 4 casas decimais
