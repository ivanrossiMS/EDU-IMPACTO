...
async function check() {
  ...
  let query = supabase.from('comunicados').select('id, titulo, destino, data, created_at').not('id', 'like', 'AD-COM-REL-COLAB-%');
  ...
  if (data) {
    data.forEach(c => console.log(c.id, '|', c.titulo, '| data:', c.data, '| created_at:', c.created_at));
  }
}
check();
