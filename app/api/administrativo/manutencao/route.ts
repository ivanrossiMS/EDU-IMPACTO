  ...
  try {
    ...
    if (Array.isArray(body)) {
      ...
      if (incomingIds.length > 0) {
        ...
        // PostgREST syntax for NOT IN: id=not.in.(id1,id2,...)
        ...
        // We use a query that matches all records (e.g., id is not null)
        const { error: delError } = await supabase
          .from('adm_manutencao')
          .delete()
          .not('id', 'is', null)
      ...
      if (rows.length > 0) {
        const { error } = await supabase.from('adm_manutencao').upsert(rows)
        if (error) throw new Error(error.message)
      }
    ...
    const { data, error } = await supabase.from('adm_manutencao').upsert(row).select().single()
    if (error) throw new Error(error.message)
