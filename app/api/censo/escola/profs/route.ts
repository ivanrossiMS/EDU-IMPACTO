...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase.from('censo_escola_data').select('*').eq('tipo', 'profs');
...
export async function POST(req: Request) {
  try {
    ...
    if (Array.isArray(body)) {
      ...
      const rows = body.map(r=>({id:r.id||crypto.randomUUID(),tipo:'profs',dados:r}));
      const {error} = await supabase.from('censo_escola_data').upsert(rows);
      if(error) throw new Error(error.message);
      return NextResponse.json({ok:true,count:rows.length})
    }
    const row={id:body.id||crypto.randomUUID(),tipo:'profs',dados:body};
