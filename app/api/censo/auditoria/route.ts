...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('censo_audit_logs').select('*')
...
export async function POST(request: Request) {
  try {
    ...
    if (Array.isArray(body)) {
      ...
      const { error } = await supabase.from('censo_audit_logs').upsert(rows)
      if (error) throw new Error(error.message)
      return NextResponse.json({ ok: true, count: rows.length })
    }

    const row = buildRowAuth(body)
    const { data, error } = await supabase.from('censo_audit_logs').upsert(row).select().single()
    if (error) throw new Error(error.message)
