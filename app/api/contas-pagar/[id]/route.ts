...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  const { data, error } = await supabaseServer.from('contas_pagar').select('*').eq('id', params.id).single()
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const row = {
      id: params.id, descricao: descricao || '', categoria: categoria || '',
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  const { error } = await supabaseServer.from('contas_pagar').delete().eq('id', params.id)
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })
