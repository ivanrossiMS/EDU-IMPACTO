...
export async function POST(request: Request) {
  try {
    ...
    if (Array.isArray(body)) {
      ...
      const { error } = await supabaseServer.from('frequencias').upsert(rows)
      if (error) return NextResponse.json({ error: error.message }, { status: 400 })
      return NextResponse.json({ ok: true, count: rows.length })
    }
    const row = buildRow(body)
    const { data, error } = await supabaseServer.from('frequencias').upsert(row).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 400 })
...
export async function DELETE(request: Request) {
  ...
  const { error } = await supabaseServer.from('frequencias').delete().eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })
