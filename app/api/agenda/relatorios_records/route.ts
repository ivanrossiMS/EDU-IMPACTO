...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase.from('relatorios_records').select('*')
    if (accessStartDate) {
...
export async function POST(request: Request) {
  try {
    ...
    if (Array.isArray(body)) {
      ...
      const { error } = await supabase.from('relatorios_records').upsert(rows)
      if (error) throw new Error(error.message)
      return NextResponse.json({ ok: true, count: rows.length })
    }

    const row = buildRowAuth(body)
    const { data, error } = await supabase.from('relatorios_records').upsert(row).select().single()
    if (error) throw new Error(error.message)
