...
export async function POST(request: Request) {
  try {
    ...
    if (Array.isArray(body)) {
      ...
      const { error } = await supabase.from('advertencias').upsert(rows)
      if (error) throw new Error(error.message)
      return NextResponse.json({ ok: true, count: rows.length })
    }

    const row = buildRowAuth(body)
    const { data, error } = await supabase.from('advertencias').upsert(row).select().single()
    if (error) throw new Error(error.message)
...
function buildRowAuth(body: any) {
  const { id, funcionario_id, ...rest } = body
  return {
