...
export async function GET() {
  ...
  try {
    ...
    const proxVenc = resContasPagarProx.data ?? []
    ...
    // Contas vencidas
    const vencidas = resContasPagarVencidas.data ?? []
    if (vencidas.length > 0) {
      const val = vencidas.reduce((s: number, c: any) => s + (Number(c.valor) || 0), 0)
      alertas.push({
        id: 'auto-cp-vencidas',
        nivel: 'critico',
        titulo: `${vencidas.length} conta${vencidas.length > 1 ? 's' : ''} a pagar vencida${vencidas.length > 1 ? 's' : ''}`,
        descricao: `Total: ${formatCurrency(val)}`,
        acao: 'Ver contas',
        link: '/financeiro/pagar',
      })
