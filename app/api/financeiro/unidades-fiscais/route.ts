...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('unidades_fiscais').select('*')
...
export async function POST(request: Request) {
  try {
    ...
    if (Array.isArray(body)) {
      ...
      const { error } = await supabase.from('unidades_fiscais').upsert(rows)
      if (error) throw new Error(error.message)
      return NextResponse.json({ ok: true, count: rows.length })
    }

    const row = buildRowAuth(body)
    const { data, error } = await supabase.from('unidades_fiscais').upsert(row).select().single()
    if (error) throw new Error(error.message)
