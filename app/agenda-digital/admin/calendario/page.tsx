...
function VisibilidadeSelector({
  ...
  return (
    <div className="ad-admin-page-container ad-mobile-optimized" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
    ...
    <div className="ad-admin-page-container ad-mobile-optimized ad-calendar-mobile-container">
      <style dangerouslySetInnerHTML={{__html: `
        @media (max-width: 768px) {
           /* Header reset overrides */
           .ad-calendar-mobile-container .page-header {
              align-items: center !important;
              margin-bottom: 24px !important;
              text-align: center !important;
           }
           .ad-calendar-mobile-container .page-title {
              font-size: 22px !important;
           }
           
           /* Fix filters area */
           .ad-calendar-filter-bar {
              flex-direction: column !important;
              align-items: stretch !important;
              padding: 16px !important;
              gap: 12px !important;
           }
           .ad-calendar-filter-bar > span:first-child {
              text-align: center !important;
           }
           .ad-calendar-filter-bar select {
              width: 100% !important;
           }
           /* Override the global header style applied by layout */
           .ad-calendar-mobile-container > div:first-child {
              text-align: left !important;
           }
           
           .ad-calendar-grid-columns {
              grid-template-columns: 1fr !important;
              gap: 16px !important;
           }
           
           /* Calendar Layout */
           .ad-calendar-grid-wrapper {
              padding: 12px !important;
              border-radius: 16px !important;
           }
           .ad-calendar-month-title {
           ...
           .ad-calendar-event-chip {
              font-size: 8px !important;
              padding: 1px 2px !important;
           }
           
           /* Side panels fix */
           .ad-calendar-side-panel {
              padding: 16px !important;
              border-radius: 16px !important;
           }
           ...
           .ad-calendar-event-card-actions {
              width: 100% !important;
      ...
      <div className="ad-calendar-grid-columns" style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20, alignItems: 'start' }}>
        {/* Calendário */}
        <div className="card ad-calendar-grid-wrapper" style={{ padding: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <button className="btn btn-ghost btn-icon" style={{ flexShrink: 0 }} onClick={() => setViewDate(new Date(year, month - 1, 1))}><ChevronLeft size={16} /></button>
            <div className="ad-calendar-month-title" style={{ fontSize: 18, fontWeight: 800, fontFamily: 'Outfit, sans-serif' }}>{MESES[month]} {year}</div>
            <button className="btn btn-ghost btn-icon" style={{ flexShrink: 0 }} onClick={() => setViewDate(new Date(year, month + 1, 1))}><ChevronRight size={16} /></button>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2 }}>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <button key={d} className="ad-calendar-day-btn" onClick={() => setSelectedDay(isSelected ? null : dateStr)}
                  ...
                  <div className="ad-calendar-day-num" style={{ fontSize: 13, fontWeight: isToday ? 800 : 500, color: isToday ? '#60a5fa' : 'hsl(var(--text-primary))', marginBottom: 2 }}>{d}</div>
                  {events.slice(0, 2).map(ev => (
                    <div key={ev.id} className="ad-calendar-event-chip" style={{ fontSize: 9, background: (ev.cor ?? TIPO_CORES[ev.tipo]) + '22', color: ev.cor ?? TIPO_CORES[ev.tipo], padding: '1px 3px', borderRadius: 3, marginBottom: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 600 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="card ad-calendar-side-panel" style={{ padding: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {selectedEvents.map(ev => (
                  <div key={ev.id} style={{ padding: '10px', background: 'hsl(var(--bg-elevated))', borderRadius: 8, borderLeft: `3px solid ${ev.cor ?? TIPO_CORES[ev.tipo]}` }}>
                    <div className="ad-calendar-event-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ fontSize: 12, fontWeight: 700 }}>{ev.titulo}</div>
                      <div className="ad-calendar-event-card-actions" style={{ display: 'flex', gap: 4 }}>
