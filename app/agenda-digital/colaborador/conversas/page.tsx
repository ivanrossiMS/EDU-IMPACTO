...
export default function ADConversasPage() {
  ...
  // Todos os colaboradores podem se comunicar em qualquer canal de equipe
  const minhasEquipes = React.useMemo(() => {
    return (equipes || []).filter((eq: any) => eq && eq.id)
  }, [equipes])
  ...
  // ── Equipes chat state
  const [activeEquipeChat, setActiveEquipeChat] = useState<any>(null) // equipe object
  const [equipeInputText, setEquipeInputText] = useState('')
  
  const messagesEndRef = useRef<HTMLDivElement>(null)
  const equipeMessagesEndRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    ...
    const filteredChats = (chatsList || []).filter(c => {
      ...
      // Excluir chats de equipe (prefixo equipe-)
  ...
  // Scroll to bottom for equipe chat
  const equipeMessages = activeEquipeChat ? messages[`equipe-${activeEquipeChat.id}`] || [] : []
  useEffect(() => {
    equipeMessagesEndRef.current?.scrollIntoView({ behavior: 'smooth' })
  }, [equipeMessages])
  ...
  // ── Send message in equipe group chat
  const handleEquipeSend = (e: React.FormEvent) => {
    e.preventDefault()
    if (!equipeInputText.trim() || !activeEquipeChat) return

    const chatKey = `equipe-${activeEquipeChat.id}`
    const newMsg = {
      id: Date.now(),
      text: equipeInputText,
      sender: 'us' as const,
      time: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
      date: new Date().toLocaleDateString('pt-BR'),
    ...
    setMessages(prev => {
      ...
      return { ...prev, [chatKey]: [...current, newMsg] }
    })

    // Upsert a chat entry in chatsList so it shows as active
    setChatsList(prev => {
      const existing = prev.find(c => String(c.id) === chatKey)
      if (existing) {
        return [
          { ...existing, preview: equipeInputText, time: newMsg.time, date: newMsg.date },
          ...prev.filter(c => String(c.id) !== chatKey)
        ]
      }
      return [
        {
          id: chatKey,
          name: activeEquipeChat.nome,
          status: 'online',
          preview: equipeInputText,
          time: newMsg.time,
          date: newMsg.date,
          unread: 1,
          tag: activeEquipeChat.nome
        },
        ...prev
      ]
    })
  ...
  return (
    <>
      ...
      <div className={`ad-chat-sidebar ${(activeChat || showNovaConversa || activeEquipeChat) ? 'mobile-hidden' : ''}`} style={{ width: 360, background: 'hsl(var(--bg-surface))', borderRight: '1px solid hsl(var(--border-subtle))', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '24px 20px 12px', borderBottom: '1px solid rgba(0,0,0,0.02)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            {activeTab === 'familias' && adConfig?.permissoes?.chat !== false && (
              <button 
          ...
          <div style={{ display: 'flex', background: 'hsl(var(--bg-main))', borderRadius: 14, padding: 4, marginBottom: 14, border: '1px solid hsl(var(--border-subtle))' }}>
            {[
              ...
              { id: 'equipes', label: 'Equipes', icon: <Shield size={13} />, count: minhasEquipes.length }
            ].map(tab => (
              <button
                key={tab.id}
                onClick={() => { setActiveTab(tab.id as any); setActiveChat(null); setActiveEquipeChat(null); setShowNovaConversa(false) }}
                style={{
                  flex: 1,
                  padding: '7px 10px',
                  borderRadius: 10,
          ...
          {activeTab === 'familias' && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ position: 'relative', zIndex: 50, marginBottom: 10 }}>
                <TurmaDropdown 
                  ...
                  <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                      ...
                      {(() => {
                        ...
                        const isEquipe = String(chat.id).startsWith('equipe-')
                        const shouldShowBadge = (isEquipe && String(lastMsg.authorId) !== String(currentUser?.id) && chat.unread > 0) || 
                                                (!isEquipe && lastMsg.sender === 'them' && chat.unread > 0)
                        if (shouldShowBadge) {
                          return (
                            <div className="badge-pulse-modern" style={{ background: '#ef4444', color: 'white', fontSize: 10, fontWeight: 800, minWidth: 18, height: 18, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px', boxShadow: '0 2px 8px rgba(239, 68, 68, 0.3)' }}>
                              {chat.unread}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 8 }}>
            {minhasEquipes.length === 0 ? (
              <div style={{ padding: 40, textAlign: 'center', color: 'hsl(var(--text-muted))' }}>
                <Shield size={36} style={{ opacity: 0.2, margin: '0 auto 12px auto' }} />
                <p style={{ fontSize: 14 }}>Você não está em nenhuma equipe ainda.</p>
              </div>
            ) : (
              minhasEquipes.map((equipe: any) => {
                const isActive = activeEquipeChat?.id === equipe.id
                const lastMsg = getEquipeLastMessage(equipe.id)
                const membros = (sysUsers || []).filter((u: any) => (equipe.membrosIds || []).includes(u.id))
                return (
                  <div
                    key={equipe.id}
                    onClick={() => { setActiveEquipeChat(equipe); setActiveChat(null); setShowNovaConversa(false) }}
                    style={{
                      padding: '14px',
                      marginBottom: 8,
                      display: 'flex',
                      ...
                      cursor: 'pointer',
                      borderRadius: 16,
                      border: isActive ? `1.5px solid ${equipe.cor || '#6366f1'}40` : '1px solid transparent',
                      background: isActive ? `${equipe.cor || '#6366f1'}08` : 'transparent',
                      transition: 'all 0.2s',
                      position: 'relative',
                      overflow: 'hidden'
                    }}
                  ...
                  >
                    {isActive && (
                      <div style={{ position: 'absolute', left: 0, top: '20%', bottom: '20%', width: 4, background: equipe.cor || '#6366f1', borderRadius: '0 4px 4px 0' }} />
                    )}
                    <div style={{ width: 46, height: 46, borderRadius: 14, background: equipe.cor || '#6366f1', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: isActive ? `0 4px 12px ${equipe.cor || '#6366f1'}40` : 'none' }}>
                      {getEquipeIcon(equipe.icone, 'white', 20)}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                        <div style={{ fontWeight: 700, fontSize: 14, color: isActive ? (equipe.cor || '#6366f1') : 'hsl(var(--text-main))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {equipe.nome}
                        </div>
                        {lastMsg && <div style={{ fontSize: 11, color: 'hsl(var(--text-muted))', flexShrink: 0 }}>{lastMsg.time}</div>}
                      </div>
                      {lastMsg ? (
                        <div style={{ fontSize: 12, color: 'hsl(var(--text-muted))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          <span style={{ fontWeight: 600 }}>{lastMsg.author?.split(' ')[0]}:</span> {lastMsg.text}
                        </div>
                      ) : (
      ...
      <div className={`ad-chat-main ${!(activeChat || showNovaConversa || activeEquipeChat) ? 'mobile-hidden' : ''}`} style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'hsl(var(--bg-main))', position: 'relative', width: '100%' }}>
          
          {/* ══ EQUIPE GROUP CHAT ══ */}
          {activeEquipeChat && !activeChat && !showNovaConversa ? (
            <>
              {/* Equipe Chat Header */}
              <div className="ad-chat-header" style={{ padding: '20px 32px', background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(16px)', borderBottom: '1px solid hsl(var(--border-subtle))', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 10 }}>
                <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                  <button className="mobile-back-btn" onClick={() => setActiveEquipeChat(null)} style={{ background: 'transparent', border: 'none', padding: 0, display: 'none', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'hsl(var(--text-main))' }}>
                    <ChevronLeft size={24} />
                  </button>
                  <div style={{ width: 52, height: 52, borderRadius: 14, background: activeEquipeChat.cor || '#6366f1', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 6px 16px ${activeEquipeChat.cor || '#6366f1'}40` }}>
                    {getEquipeIcon(activeEquipeChat.icone, 'white', 22)}
                  </div>
                  <div>
                    <div style={{ fontWeight: 800, fontSize: 18, color: 'hsl(var(--text-main))', marginBottom: 2 }}>{activeEquipeChat.nome}</div>
                    <div style={{ fontSize: 12, color: 'hsl(var(--text-muted))', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600 }}>
                      ...
                      <span style={{ padding: '2px 8px', borderRadius: 6, background: `${activeEquipeChat.cor || '#6366f1'}15`, color: activeEquipeChat.cor || '#6366f1', fontSize: 11, fontWeight: 700 }}>
                        {(sysUsers || []).filter((u: any) => (activeEquipeChat.membrosIds || []).includes(u.id)).length} membros
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  {(sysUsers || []).filter((u: any) => (activeEquipeChat.membrosIds || []).includes(u.id)).slice(0, 5).map((u: any, i: number) => (
                    <div key={u.id} title={u.nome} style={{ width: 32, height: 32, borderRadius: '50%', background: activeEquipeChat.cor || '#6366f1', border: '2px solid white', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 11, fontWeight: 800, marginLeft: i > 0 ? -8 : 0, zIndex: 10 - i }}>
                      {getInitials(u.nome || '?').slice(0, 1)}
                    </div>
                  ))}
                </div>
              </div>

              {/* Equipe Messages Area */}
              <div className="ad-chat-messages-area" style={{ flex: 1, padding: '32px 40px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
                {/* Canal header */}
                <div style={{ textAlign: 'center', margin: '10px 0 20px' }}>
                  <span style={{ background: 'hsl(var(--bg-surface))', padding: '6px 16px', borderRadius: 20, fontSize: 11, fontWeight: 700, color: 'hsl(var(--text-muted))', border: '1px solid hsl(var(--border-subtle))' }}>
                    🔒 Canal interno · apenas membros da equipe
                  </span>
                </div>

                {equipeMessages.length === 0 && (
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'hsl(var(--text-muted))' }}>
                    <div style={{ width: 64, height: 64, borderRadius: 32, background: `${activeEquipeChat.cor || '#6366f1'}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
                      {getEquipeIcon(activeEquipeChat.icone, activeEquipeChat.cor || '#6366f1', 28)}
                    </div>
                    <p style={{ fontWeight: 700, fontSize: 16, marginBottom: 4, color: 'hsl(var(--text-main))' }}>Canal {activeEquipeChat.nome}</p>
                    <p style={{ fontSize: 14, textAlign: 'center', maxWidth: 360 }}>Este é o início do canal. Envie a primeira mensagem para a equipe!</p>
                  </div>
                )}

                {equipeMessages.map((msg: any) => {
                  ...
                  return (
                    <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start', marginBottom: 16 }}>
                      {!isMe && (
                        <div style={{ fontSize: 11, fontWeight: 700, color: '#64748b', marginBottom: 4, marginLeft: 44 }}>
                          {msg.author || 'Colaborador'} • {msg.time}
                        </div>
                      )}
                      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: isMe ? 'flex-end' : 'flex-start', width: '100%' }}>
                        {!isMe && (
                          <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg, #FF0080, #7928ca)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 12, marginRight: 12, flexShrink: 0 }}>
                            {getInitials(msg.author || 'C')}
                          </div>
                        )}
                        <div style={{ 
                          background: isMe ? `linear-gradient(135deg, ${activeEquipeChat.cor || '#6366f1'} 0%, ${activeEquipeChat.cor || '#4f46e5'}dd 100%)` : '#f1f5f9', 
                          ...
                          fontSize: 14,
                          lineHeight: 1.5,
                          boxShadow: isMe ? '0 4px 12px rgba(121, 40, 202, 0.2)' : '0 2px 8px rgba(0,0,0,0.05)'
                        }}>
                          {msg.text}
                        </div>
                      </div>
                      {isMe && (
                        <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 6, fontWeight: 600 }}>
                          {msg.time}
                        </div>
                      )}
                    </div>
              ...
              <div className="ad-chat-input-area" style={{ padding: '20px 40px', background: 'hsl(var(--bg-surface))', borderTop: '1px solid hsl(var(--border-subtle))' }}>
                <form onSubmit={handleEquipeSend} style={{ display: 'flex', gap: 14 }}>
                  <input 
                    value={equipeInputText}
                    onChange={e => setEquipeInputText(e.target.value)}
                    className="form-input" 
                    placeholder={`Mensagem para ${activeEquipeChat.nome}...`}
                  ...
                  <button 
                    ...
                    style={{ 
                      ...
                      alignItems: 'center', 
                      justifyContent: 'center', 
                      background: equipeInputText.trim() ? `linear-gradient(135deg, ${activeEquipeChat.cor || '#6366f1'}, ${activeEquipeChat.cor || '#4f46e5'}dd)` : 'hsl(var(--bg-surface-alt))', 
                      color: equipeInputText.trim() ? 'white' : 'hsl(var(--text-muted))', 
                      border: 'none', 
                      boxShadow: equipeInputText.trim() ? `0 4px 14px ${activeEquipeChat.cor || '#6366f1'}50` : 'none', 
                      transition: 'all 0.2s', 
                      cursor: equipeInputText.trim() ? 'pointer' : 'not-allowed' 
                    }}
                    disabled={!equipeInputText.trim()}
                  >
