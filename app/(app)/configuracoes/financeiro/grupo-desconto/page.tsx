'use client'
import { useData, ConfigGrupoDesconto, newId } from '@/lib/dataContext'
import { useState, useMemo } from 'react'
import { Plus, Edit2, Trash2, Check, Percent, Search, Sparkles, Shield } from 'lucide-react'

// ── Grupos padrão pré-definidos ────────────────────────────────────────────
...
// Cor dinâmica baseada no valor do desconto
...
export default function GrupoDescontoPage() {
  const { cfgGruposDesconto, setCfgGruposDesconto, cfgEventos } = useData()
  ...
  const handleDelete = (id: string) => setCfgGruposDesconto(prev => prev.filter(g => g.id !== id))
  const handleSave = () => {
  ...
  // Carrega grupos padrão (sem duplicar pelo nome)
  const carregarPadroes = () => {
    ...
    const gerados = novos.map(g => {
      // usa o codigo do padrão se disponível e não duplicado; caso contrário, gera um novo
      let codigo = g.codigo
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Total', value: cfgGruposDesconto.length, color: '#3b82f6' },
          { label: 'Ativos agora', value: ativos.length, color: '#10b981' },
          { label: 'Expirados', value: expirados.length, color: '#6b7280' },
        ].map(k => (
          ...
          {gruposFiltrados.map(g => {
            ...
            return (
              <div key={g.id} className="card" style={{ padding: '20px', borderTop: `3px solid ${vcor}`, position: 'relative', overflow: 'hidden' }}>
                {/* Watermark de valor — pointerEvents: none para não bloquear botões */}
