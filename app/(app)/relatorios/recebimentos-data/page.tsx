...
type ModeloRelatorio = 'resumo' | 'por-metodo' | 'por-evento' | 'por-aluno' | 'com-responsavel' | 'completo'
type OpcaoParcelas = 'todas' | 'em-aberto'
type OrdenarPor = 'nome' | 'codigo' | 'turma' | 'evento' | 'vencimento' | 'dataPagamento'
...
function PrintLayout({ parcelas, filters, modelo, kpis, kpiLiquido }: {
  parcelas: Parcela[]
  filters: Filters
  modelo: ModeloRelatorio
  kpis: { total: number; valor: number; jurosMulta: number; desconto: number; saldo: number; alunos: number; qtdParcelas: number }
  kpiLiquido: number
}) {
  ...
  return (
    <div id="print-layout" style={P.page}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p style={{ fontSize: 11, color: '#555', margin: '2px 0 0' }}>
              RECEBIMENTOS POR DATA — {modelo === 'resumo' ? 'Resumo' : modelo === 'por-evento' ? 'Por Evento' : modelo === 'por-aluno' ? 'Por Aluno' : 'Com Dados do Responsável'}
            </p>
      ...
      <div style={{ marginTop: 14, border: '2px solid #0ea5e9', borderRadius: 6, padding: '8px 14px', background: '#f0f9ff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ fontSize: 9, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.07em' }}>TOTAL GERAL — RECEBIMENTOS POR DATA</div>
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
          ...
          <div style={{ textAlign: 'right' }}><div style={{ fontSize: 8, color: '#dc2626', fontWeight: 700, textTransform: 'uppercase' }}>Juros/Multa</div><div style={{ fontSize: 13, fontWeight: 800, fontFamily: 'monospace', color: '#dc2626' }}>{fmtCur(kpis.jurosMulta)}</div></div>
          ...
          <div style={{ textAlign: 'right', borderLeft: '2px solid #0ea5e9', paddingLeft: 16 }}><div style={{ fontSize: 8, color: '#0ea5e9', fontWeight: 700, textTransform: 'uppercase' }}>TOTAL (LÍQUIDO)</div><div style={{ fontSize: 20, fontWeight: 900, fontFamily: 'monospace', color: '#0ea5e9' }}>{fmtCur(kpiLiquido)}</div></div>
        </div>
      </div>
...
/* ─── VISÕES ADICIONAIS: COMPLETA E POR MÉTODO ───────────────────────── */

function ViewCompleta({ parcelas, sortField, sortDir, onSort, filters }: { parcelas: Parcela[]; sortField: string; sortDir: 'asc' | 'desc'; onSort: (f: string) => void; filters: Filters }) {
  const totalLiq = parcelas.reduce((s, p) => s + p.valor + (filters.somarJurosMulta ? (p.juros||0)+(p.multa||0) : 0) - (filters.somarDesconto ? p.desconto : 0), 0)
  return (
    <div style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid hsl(var(--border-subtle))', background: 'hsl(var(--bg-surface))', overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead><tr>
          ...
          <Th f="vencimento" label="Venc" sortField={sortField} sortDir={sortDir} onSort={onSort} align="center" />
          <Th f="dataPagamento" label="Pgto" sortField={sortField} sortDir={sortDir} onSort={onSort} align="center" />
          <Th f="formaPagamento" label="Método" sortField={sortField} sortDir={sortDir} onSort={onSort} align="center" />
          <Th f="valor" label="Original" sortField={sortField} sortDir={sortDir} onSort={onSort} align="right" />
          ...
          <Th f="saldo" label="Liq" sortField={sortField} sortDir={sortDir} onSort={onSort} align="right" />
          {filters.mostrarObservacoes && <th style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase', color: 'hsl(var(--text-muted))', padding: '7px 10px', textAlign: 'left', borderBottom: '1px solid hsl(var(--border-subtle))', background: 'hsl(var(--bg-elevated))' }}>Obs.</th>}
        </tr></thead>
        <tbody>{parcelas.map((p, idx) => {
          ...
          return (
            <tr key={`${p.alunoId}-${p.parcela}-${idx}`} style={{ background: idx % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.012)' }}>
              <td style={{ padding: '7px 10px', fontSize: 11, fontWeight: 600 }}>{p.nome}</td>
              ...
              <td style={{ padding: '7px 10px', fontSize: 11, textAlign: 'center', fontWeight: 600 }}>{fmtDate(p.dataPagamento)}</td>
              <td style={{ padding: '7px 10px', fontSize: 11, textAlign: 'center', color: '#0369a1' }}>{p.formaPagamento || '—'}</td>
              ...
              {filters.mostrarObservacoes && <td style={{ padding: '7px 10px', fontSize: 10, color: 'hsl(var(--text-muted))', maxWidth: 150, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={p.observacaoBaixa}>{p.observacaoBaixa || '—'}</td>}
            </tr>
          )
        })}</tbody>
        <tfoot><tr style={{ background: 'hsl(var(--bg-elevated))', borderTop: '2px solid hsl(var(--border-default))' }}>
          <td colSpan={6} style={{ padding: '8px 12px', fontSize: 11, fontWeight: 700, color: 'hsl(var(--text-secondary))' }}>Total (Lista Completa)</td>
          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 700, fontSize: 12, fontFamily: 'monospace' }}>{fmtCur(parcelas.reduce((s,p) => s+p.valor, 0))}</td>
          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 700, fontSize: 11, color: ACCENT4 }}>{fmtCur(parcelas.reduce((s, x) => s + (x.juros||0) + (x.multa||0), 0))}</td>
          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 700, fontSize: 12, fontFamily: 'monospace', color: ACCENT2 }}>{fmtCur(parcelas.reduce((s,p)=>s+p.desconto,0))}</td>
...
function ViewPorMetodo({ parcelas, sortField, sortDir, onSort, filters }: { parcelas: Parcela[]; sortField: string; sortDir: 'asc' | 'desc'; onSort: (f: string) => void; filters: Filters }) {
  const byMetodo = useMemo(() => {
    const map: Record<string, Parcela[]> = {}
    parcelas.forEach(p => { const k = p.formaPagamento || 'Não Informado'; if (!map[k]) map[k] = []; map[k].push(p) })
    return Object.entries(map).sort(([a], [b]) => b[1].length - a[1].length).map(([k, items]) => {
  ...
  return (
    <div>
      {byMetodo.map(({ key, items, totalValor, totalLiq }) => {
        ...
        return (
          <div key={key} style={{ marginBottom: 10, borderRadius: 12, overflow: 'hidden', border: '1px solid hsl(var(--border-subtle))', background: 'hsl(var(--bg-surface))' }}>
            <button onClick={() => toggle(key)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', border: 'none', borderBottom: isOpen ? '1px solid hsl(var(--border-subtle))' : 'none', borderLeft: `4px solid #10b981`, background: isOpen ? 'rgba(16,185,129,0.06)' : 'transparent', cursor: 'pointer', transition: 'background 0.15s', textAlign: 'left' }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 800, color: 'hsl(var(--text-primary))', flex: 1, display: 'flex', alignItems: 'center', gap: 8 }}><Layers size={13} color="#10b981"/> {key.toUpperCase()}</span>
              <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 5, background: 'rgba(16,185,129,0.12)', color: '#10b981' }}>{items.length} oper.</span>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 9, color: '#10b981', fontWeight: 600, textTransform: 'uppercase' }}>Volume</div>
                <div style={{ fontSize: 14, fontWeight: 800, color: '#059669', fontFamily: 'monospace' }}>{fmtCur(Math.max(0, totalLiq))}</div>
              </div>
            </button>
            {isOpen && (
              <div style={{ overflowX: 'auto', padding: 12 }}>
                <ViewCompleta parcelas={items} sortField={sortField} sortDir={sortDir} onSort={onSort} filters={filters} />
              </div>
            )}
          </div>
        )
...
const DEFAULT_FILTERS: Filters = {
  ...
  dataFim: thisYearEnd(),
  grupoAluno: '', nivelEnsino: '', statusPagamento: 'pago', formaPagamento: '',
...
export default function RecebimentosDataPage() {
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      ...
      // Sem filtro restritivo de Opção de parcelas - Exibindo todas (pagas, abertas, vencidas)
      // Se houvesse filtro de status ele operaria aqui.
  ...
  const kpis = useMemo(() => ({
    ...
    saldoResidual: sorted.reduce((s, p) => s + (Number(p.saldo) || 0), 0),
  ...
  // Excel export
  const exportXLSX = () => {
    const defaultHeaders = ['Código', 'Aluno', 'Turma', 'Série', 'Unidade', 'Responsável', 'Evento', 'Parcela', 'Vencimento', 'DataPgto', 'MetodoPgto', 'Valor Original', 'Juros', 'Multa', 'Desconto', 'Líquido', 'Sdo Devedor', 'Status']
    if (filters.mostrarObservacoes) defaultHeaders.push('Observações')
    const rows: any[][] = [['RELATÓRIO DE RECEBIMENTOS POR DATA'], [`Período: ${fmtDate(filters.dataInicio)} a ${fmtDate(filters.dataFim)}`], [], defaultHeaders]
    sorted.forEach(p => {
      const liq = p.valor + (filters.somarJurosMulta ? (p.juros || 0) + (p.multa || 0) : 0) - (filters.somarDesconto ? p.desconto : 0)
      const r = [p.codigo, p.nome, p.turma, p.serie, p.unidade, p.responsavelFinanceiro, p.evento, p.parcela, fmtDate(p.vencimento), fmtDate(p.dataPagamento), p.formaPagamento, p.valor, p.juros, p.multa, p.desconto, Math.max(0, liq), p.saldo, p.statusFinanceiro]
      if (filters.mostrarObservacoes) r.push(p.observacaoBaixa)
      rows.push(r)
    })
    rows.push([])
    const footer = ['', '', '', '', '', '', '', '', '', '', 'TOTAL:', kpis.valor, kpis.jurosMulta, '', kpis.desconto, kpiLiquido, kpis.saldoResidual, '']
    if (filters.mostrarObservacoes) footer.push('')
    rows.push(footer)
    const ws = XLSX.utils.aoa_to_sheet(rows)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'Recebimentos')
    XLSX.writeFile(wb, `recebimentos-${filters.dataInicio || 'completo'}.xlsx`)
  }
...
<html lang="pt-BR"><head>
  <meta charset="UTF-8">
  <title>Recebimentos por Data</title>
  <style>
  ...
  const modeloLabels: Record<ModeloRelatorio, string> = {
    'resumo': 'Resumo Data/Mês',
    'por-metodo': 'Método Pagamento',
    'por-evento': 'Por Evento',
    'por-aluno': 'Agrupado por Aluno',
    'com-responsavel': 'Dados do Resp.',
    ...
    <>
      ...
      {hasLoaded && (
        <div id="print-root" ref={printRef} style={{ display: 'none' }}>
          <PrintLayout parcelas={sorted} filters={filters} modelo={modelo} kpis={kpis as any} kpiLiquido={kpiLiquido} />
        </div>
      )}
      ...
      <div id="screen-root" style={{ maxWidth: 1440, margin: '0 auto', padding: '0 0 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 22, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 3 }}>
              ...
              <h1 className="page-title" style={{ fontSize: 20, margin: 0 }}>Recebimentos por Data</h1>
          ...
          <div className="card" style={{ padding: '20px 22px', marginBottom: 18 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))', gap: 10, marginBottom: 14 }}>
              ...
              <div style={{ gridColumn: '1 / -1', display: 'flex', gap: 12, marginTop: 4, paddingTop: 14, borderTop: '1px solid hsl(var(--border-subtle))' }}>
                <button
                  type="button"
                  onClick={() => setFilters(f => ({ ...f, mostrarObservacoes: !f.mostrarObservacoes }))}
                  style={{
                ...
                >
                  <Eye size={12} style={{ color: filters.mostrarObservacoes ? '#10b981' : 'hsl(var(--text-muted))' }} />
                  Mostrar Observações da Baixa
                </button>
              </div>
        ...
        {hasLoaded && (
          <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {(['resumo', 'por-metodo', 'por-evento', 'por-aluno', 'com-responsavel', 'completo'] as ModeloRelatorio[]).map(m => (
              <button key={m} onClick={() => setModelo(m)} style={{ fontSize: 11, fontWeight: 700, padding: '6px 14px', borderRadius: 6, cursor: 'pointer', border: modelo === m ? `1.5px solid ${ACCENT}` : '1px solid hsl(var(--border-default))', background: modelo === m ? `${ACCENT}15` : 'hsl(var(--bg-surface))', color: modelo === m ? ACCENT : 'hsl(var(--text-secondary))', transition: 'all 0.15s' }}>
                {m === 'resumo' && '📊 '} {m === 'por-metodo' && '💳 '} {m === 'completo' && '📝 '} {m === 'por-evento' && '🏷️ '} {m === 'por-aluno' && '👤 '} {m === 'com-responsavel' && '📋 '}
                {modeloLabels[m]}
              </button>
            ))}
        ...
        {!hasLoaded && !loading && (
          <div className="card" style={{ padding: '60px 40px', textAlign: 'center', border: '1px dashed hsl(var(--border-subtle))' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: 'hsl(var(--text-primary))', marginBottom: 6 }}>Relatório de Recebimentos por Data</div>
        ...
        {hasLoaded && !loading && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 16 }}>
              <KpiCard label="Total Recebido (Líquido)" value={fmtCur(kpiLiquido)} icon={<DollarSign size={18} />} color={ACCENT} sub={`${kpis.qtdParcelas} parcelas na visão atual`} />
              <KpiCard label="Descontos Totais" value={fmtCur(kpis.desconto)} icon={<Tag size={18} />} color={ACCENT2} sub="deduções aplicadas" />
              <KpiCard label="Juros e Multa" value={fmtCur(kpis.jurosMulta)} icon={<TrendingUp size={18} />} color={ACCENT4} sub="acréscimos aplicados" />
              <KpiCard label="Alunos Identificados" value={String(kpis.alunos)} icon={<Users size={18} />} color="#7c3aed" sub="tamanho do grupo filtrado" />
            </div>
            ...
            {monthlyData.length > 1 && (
              <div className="card" style={{ padding: '14px 18px', marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: 'hsl(var(--text-secondary))', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Recebimentos por Mês</span>
                </div>
            ...
            {sorted.length > 0 && (
              <>
                ...
                {modelo === 'por-metodo' && <ViewPorMetodo parcelas={sorted} sortField={sortField} sortDir={sortDir} onSort={handleSort} filters={filters} />}
                {modelo === 'completo' && <ViewCompleta parcelas={sorted} sortField={sortField} sortDir={sortDir} onSort={handleSort} filters={filters} />}
