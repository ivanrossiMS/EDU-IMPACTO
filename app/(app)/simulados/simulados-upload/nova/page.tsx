...
import { 
  ...
  Upload, Sparkles, CheckCircle, ChevronUp, ChevronDown, Layers, Info, Check, X 
} from 'lucide-react'
...
export default function NovaSimuladoUploadPage() {
  ...
  const [assignments, setAssignments] = useState<any[]>([
    { id: Date.now().toString(), disciplinaId: '', disciplinaNome: '', professorId: '', professorNome: '', qtdQuestoes: 10, area: undefined }
  ])
  ...
  useEffect(() => {
    async function load() {
      ...
      if (adaptarDe) {
        const { data: oldSimulado } = await (supabase as any).from('simulados_upload').select('*').eq('id', adaptarDe).single()
        if (oldSimulado) {
          setTitulo((oldSimulado.titulo || '') + ' - Adaptado')
          if (oldSimulado.descricao) setDescricao(oldSimulado.descricao)
          if (oldSimulado.data_aplicacao) setDataAplicacao(oldSimulado.data_aplicacao)
          if (oldSimulado.id_bimestre) setBimestreId(oldSimulado.id_bimestre)
          if (oldSimulado.series) setSeries(Array.isArray(oldSimulado.series) ? oldSimulado.series : [oldSimulado.series])
          if (oldSimulado.data_limite_upload) setDataLimiteUpload(oldSimulado.data_limite_upload)
          if (oldSimulado.valor) setValor(oldSimulado.valor.toString())
          
          const { data: reqs } = await (supabase as any).from('simulados_upload_requisicoes').select('*').eq('id_simulado_upload', adaptarDe)
  ...
  const handleApplyPreset = () => {
    ...
    if (presetMode === 'replace') {
      ...
      setAssignments(prev => {
        ...
        const cleanedPrev = prev.filter(a => a.disciplinaId || a.professorId)
        return [...cleanedPrev, ...(toAdd.length > 0 ? toAdd : newItems)]
      })
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const reqs = assignments.map(a => ({
        id_simulado_upload: simuladoData.id,
      ...
      const { error: reqError } = await (supabase as any).from('simulados_upload_requisicoes').insert(reqs)
      ...
      console.error(e)
      alert('Erro ao criar simulado: ' + e.message)
    } finally {
      setLoading(false)
    }
  }
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              <h1 style={{ fontSize: 24, fontWeight: 800, color: 'hsl(var(--text-primary))', margin: 0 }}>Novo Simulado via Upload</h1>
          ...
          >
            <Save size={18} /> {loading ? 'Criando...' : 'Criar Simulado'}
          </motion.button>
        </div>
        ...
        <div style={{ background: 'hsl(var(--bg-surface))', border: '1px solid hsl(var(--border-subtle))', borderRadius: 20, padding: 32, marginBottom: 24 }}>
          <h3 style={{ color: 'hsl(var(--text-primary))', fontSize: 16, fontWeight: 700, margin: '0 0 24px', display: 'flex', alignItems: 'center', gap: 8 }}>
            <BookOpen size={18} color="#8b5cf6" /> Informações da Simulado
          </h3>
        ...
        {successModal && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
              ...
              <h2 style={{ fontSize: 24, fontWeight: 800, color: 'hsl(var(--text-primary))', margin: '0 0 12px' }}>Simulado Criada!</h2>
              <p style={{ color: 'hsl(var(--text-secondary))', fontSize: 15, lineHeight: 1.5, margin: '0 0 32px' }}>
                A simulado foi criada com sucesso. Os professores vinculados já podem acessar e enviar as questões.
              </p>
