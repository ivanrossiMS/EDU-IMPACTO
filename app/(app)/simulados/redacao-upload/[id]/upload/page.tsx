...
export default function UploadRedaçãoPage() {
  ...
  const handleSave = async (updatedQuestoes?: any[], actionType?: 'enviar_revisao' | 'aredacaor') => {
    ...
    if (currentUser?.perfil === 'Professor' && myAssignment && currentQs.length > myAssignment.qtd_questoes) {
      setAlertModal({ open: true, message: `Você não pode salvar. Estão liberadas apenas ${myAssignment.qtd_questoes} questões para você nesta redacao. Edite ou exclua algumas questões para prosseguir.` });
      return;
    }
