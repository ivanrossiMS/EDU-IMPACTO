...
export default function EditarRedaçãoUploadPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        const [bimRes, discRes, redacaoRes] = await Promise.all([
          ...
          (supabase as any).from('redacao_upload').select('*, redacao_upload_requisicoes(*)').eq('id', params.id).single()
        ])
        
        setBimestres(bimRes.data || [])
        setDisciplinas(discRes.data || [])
        
        const redacaoData = redacaoRes.data
        if (redacaoData) {
          setTitulo(redacaoData.titulo || '')
          setDescricao(redacaoData.descricao || '')
          setDataAplicacao(redacaoData.data_aplicacao || '')
          setBimestreId(redacaoData.id_bimestre || '')
          setSeries(redacaoData.series || [])
          setInstrucoes(redacaoData.instrucoes || '')
          setDataLimiteUpload(redacaoData.data_limite_upload || '')
          if (redacaoData.valor) setValor(redacaoData.valor.toString())
          
          if (redacaoData.redacao_upload_requisicoes) {
            setAssignments(redacaoData.redacao_upload_requisicoes.map((r: any) => ({
        ...
        console.error(e)
        alert('Erro ao carregar dados da redacao.')
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data: existingReqs } = await (supabase as any)
        ...
        .eq('id_redacao_upload', params.id)
      ...
      if (reqsToDelete.length > 0) {
        await (supabase as any).from('redacao_upload_requisicoes').delete().in('id', reqsToDelete.map((r: any) => r.id))
      }

      // Fetch current questoes_json and filter out questions belonging to deleted requisitions/disciplines
      const { data: currentRedacao } = await (supabase as any)
        .from('redacao_upload')
        .select('questoes_json')
        .eq('id', params.id)
        .single()

      let currentQuestions = Array.isArray(currentRedacao?.questoes_json) ? currentRedacao.questoes_json : []
      ...
      const { error: redacaoError } = await (supabase as any)
        .from('redacao_upload')
        .update(payload)
        .eq('id', params.id)
      ...
      for (const a of assignments) {
        ...
        if (a.reqId) {
          await (supabase as any).from('redacao_upload_requisicoes').update(reqPayload).eq('id', a.reqId)
        } else {
          await (supabase as any).from('redacao_upload_requisicoes').insert([{ ...reqPayload, status: 'pendente' }])
        }
      }
      ...
      console.error(e)
      alert('Erro ao atualizar redacao: ' + e.message)
    } finally {
      setLoading(false)
    }
  }
  ...
  if (initialLoading) {
    return <div style={{ padding: 40, textAlign: 'center' }}>Carregando dados da redacao...</div>
  }
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              <h1 style={{ fontSize: 24, fontWeight: 800, color: 'hsl(var(--text-primary))', margin: 0 }}>Editar Redação</h1>
            ...
            {assignments.map((a, i) => {
              ...
              return (
                <motion.div key={a.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                  style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 14, alignItems: 'flex-end', background: 'hsl(var(--bg-app))', padding: '16px 20px', borderRadius: 14, border: '1px solid hsl(var(--border-subtle))' }}>
                  <div>
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
              ...
              <p style={{ color: 'hsl(var(--text-secondary))', fontSize: 14, lineHeight: 1.5, margin: '0 0 24px' }}>
                ...
                <br /><br />
                <span style={{ color: '#ef4444', fontWeight: 700 }}>Atenção:</span> A atribuição e <strong>todas as propostas/questões já enviadas ou cadastradas para ela serão excluídas definitivamente</strong> desta redação ao salvar.
              </p>
        ...
        {successModal && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
              ...
              <h2 style={{ fontSize: 24, fontWeight: 800, color: 'hsl(var(--text-primary))', margin: '0 0 12px' }}>Redação Atualizada!</h2>
              <p style={{ color: 'hsl(var(--text-secondary))', fontSize: 15, lineHeight: 1.5, margin: '0 0 32px' }}>
                As informações e atribuições da redacao foram salvas com sucesso.
              </p>
