...
export default function VerRedaçãoUploadPage() {
  ...
  const updateStatus = async (status: string) => {
    setSaving(true)
    try {
      await (supabase as any).from('redacao_upload').update({ status }).eq('id', redacaoId)
      setRedação((prev: any) => ({ ...prev, status }))
      setSuccessMessage(`Redação ${status === 'aredacaodo' ? 'aredacaoda' : status === 'reredacaodo' ? 'reredacaoda' : 'publicada'} com sucesso!`)
  ...
  const statusConfig: Record<string, { label: string; color: string; bg: string }> = {
    ...
    aredacaodo: { label: 'Aredacaodo', color: '#10b981', bg: 'rgba(16,185,129,0.1)' },
  ...
  if (!redacao) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'hsl(var(--text-secondary))' }}>Redação não encontrada.</div>
  }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <h1 style={{ fontSize: 24, fontWeight: 800, color: 'hsl(var(--text-primary))', margin: 0, letterSpacing: '-0.02em' }}>{redacao.titulo}</h1>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
              {redacao.data_aplicacao && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, background: 'hsl(var(--bg-surface))', border: '1px solid hsl(var(--border-subtle))', fontSize: 12, color: 'hsl(var(--text-secondary))', fontWeight: 600 }}>
                  <Calendar size={14} color="#8b5cf6" /> Aplicação: {new Date(redacao.data_aplicacao + 'T00:00:00').toLocaleDateString('pt-BR')}
                </div>
              )}
              {redacao.data_limite_upload && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, background: 'hsl(var(--bg-surface))', border: '1px solid hsl(var(--border-subtle))', fontSize: 12, color: 'hsl(var(--text-secondary))', fontWeight: 600 }}>
                  <Clock size={14} color="#f59e0b" /> Prazo: {new Date(redacao.data_limite_upload + 'T00:00:00').toLocaleDateString('pt-BR')}
                </div>
              )}
              {redacao.series && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, background: 'hsl(var(--bg-surface))', border: '1px solid hsl(var(--border-subtle))', fontSize: 12, color: 'hsl(var(--text-secondary))', fontWeight: 600 }}>
                  <Users size={14} color="#3b82f6" /> {Array.isArray(redacao.series) ? redacao.series.join(', ') : redacao.series}
                </div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, background: 'hsl(var(--bg-surface))', border: '1px solid hsl(var(--border-subtle))', fontSize: 12, color: 'hsl(var(--text-secondary))', fontWeight: 600 }}>
                <FileText size={14} color="#10b981" /> {redacao.questoes_count || 0} questões
              </div>
            </div>
          </div>
        </div>
              ...
              <div style={{ marginTop: 20, padding: '14px 16px', background: 'rgba(139,92,246,0.06)', border: '1px solid rgba(139,92,246,0.15)', borderRadius: 12 }}>
                ...
                <div style={{ fontSize: 13, color: 'hsl(var(--text-primary))' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ fontWeight: 700, color: '#10b981' }}>{requisicoes.filter(r => r.status === 'enviado' || r.status === 'aredacaodo').length}</span>
                  </div>
                </div>
              </div>
            )}
