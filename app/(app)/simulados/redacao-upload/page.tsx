...
export default function UploadRedaçõesGerenciamentoPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (redacoesData.length > 0) {
        try {
          const reqsRes = await fetch(`/api/redacao-upload/requisicoes?redacaoIds=${redacoesData.map((p: any) => p.id).join(',')}`)
          if (reqsRes.ok) {
  ...
  const handleAdaptar = async (redacao: any) => {
    try {
      const newTitle = redacao.titulo + ' - Adaptado'
      const { data: newRedação, error } = await (supabase as any).from('redacao_upload').insert({
        titulo: newTitle,
        id_bimestre: redacao.id_bimestre,
        series: redacao.series,
        data_aplicacao: redacao.data_aplicacao,
        data_limite_upload: redacao.data_limite_upload,
        questoes_json: redacao.questoes_json || [],
        questoes_count: redacao.questoes_count || 0,
        status: 'em_revisao'
      }).select().single()
  
      if (error || !newRedação) {
        alert('Erro ao adaptar redacao')
        return
      }
  
      // Clonar requisicoes
      if (redacao.redacao_upload_requisicoes && redacao.redacao_upload_requisicoes.length > 0) {
        let reqsToClone = redacao.redacao_upload_requisicoes;
        ...
        if (reqsToClone.length > 0) {
           const insertReqs = reqsToClone.map((r: any) => ({
             id_redacao_upload: newRedação.id,
           ...
           await (supabase as any).from('redacao_upload_requisicoes').insert(insertReqs)
        }
      } else if (currentUserPerfil === 'Professor') {
        // Fallback
        await (supabase as any).from('redacao_upload_requisicoes').insert({
          id_redacao_upload: newRedação.id,
          id_professor: currentUser?.id,
          professor_nome: currentUser?.nome,
          id_disciplina: null,
          disciplina_nome: 'Geral',
          qtd_questoes: redacao.questoes_count || 0,
          status: 'enviado'
        })
      }
  
      // Redirect to edit
      window.location.href = `/simulados/redacao-upload/${newRedação.id}/upload`
    } catch (e: any) {
      alert('Erro ao adaptar redacao: ' + e.message)
    }
  }

  const isProfView = currentUserPerfil === 'Professor'

  const filtered = redacoes.filter(p => {
    ...
    <div style={{ padding: '32px 40px', maxWidth: 1200, margin: '0 auto' }}>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
              ...
              <div>
                ...
                <p style={{ color: 'hsl(var(--text-secondary))', margin: 0, fontSize: 14 }}>
                  {isCoord ? 'Crie e gerencie redacoes que serão enviadas por professores' : 'Visualize e envie suas redacoes em formato DOCX ou PDF'}
                </p>
              </div>
            </div>
          </div>
        ...
        {isCoord && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
            {[
              { label: 'Total de Redações', value: stats.total, color: '#8b5cf6', bg: 'rgba(139,92,246,0.08)', icon: Layers },
              ...
              { label: 'Aredacaodas', value: stats.aredacaodo, color: '#10b981', bg: 'rgba(16,185,129,0.08)', icon: CheckCircle },
            ].map((s, i) => (
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 32, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 280px', position: 'relative' }}>
            ...
            <input
              ...
              placeholder="Buscar por título da redacao..."
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            ...
            >
              ...
              <option value="aredacaodo">Aredacaodo</option>
              <option value="reredacaodo">Reredacaodo</option>
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                        <span style={{ fontSize: 18, fontWeight: 800, color: 'hsl(var(--text-primary))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{redacao.titulo}</span>
                      ...
                      <div style={{ display: 'flex', gap: 16, color: 'hsl(var(--text-secondary))', fontSize: 12, marginBottom: 12 }}>
                        {redacao.id_bimestre && (
                           <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                             <BookOpen size={12} /> {bimestres.find(b => b.id === redacao.id_bimestre)?.nome || 'Bimestre'}
                           </span>
                        )}
                        {redacao.data_aplicacao && (
                          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            <Calendar size={12} /> Aplicação: {redacao.data_aplicacao.split('-').reverse().join('/')}
                          </span>
                        )}
                        <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: redacao.data_limite_upload && new Date(redacao.data_limite_upload) < new Date() ? '#ef4444' : 'inherit' }}>
                          <Clock size={12} /> Prazo: {redacao.data_limite_upload ? redacao.data_limite_upload.split('-').reverse().join('/') : 'Não definido'}
                        </span>
                      ...
                      {redacao.redacao_upload_requisicoes && redacao.redacao_upload_requisicoes.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
                          {redacao.redacao_upload_requisicoes.map((req: any) => {
                            const uploaded = Array.isArray(redacao.questoes_json) 
                              ? redacao.questoes_json.filter((q: any) => q.id_professor === req.id_professor).length 
                              : 0;
                            ...
                            if (canClick) {
                              return (
                                <Link key={req.id} href={`/simulados/redacao-upload/${redacao.id}/upload?prof=${req.id_professor}`} style={cardStyle as any} {...hoverProps}>
                                  {cardContent}
                                </Link>
                              )
                            }
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8, flexShrink: 0 }}>
                      ...
                      <button onClick={() => handleAdaptar(redacao)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px 16px', borderRadius: 10, background: 'rgba(59,130,246,0.1)', color: '#3b82f6', fontSize: 13, fontWeight: 600, border: '1px solid rgba(59,130,246,0.2)', cursor: 'pointer' }}>
      ...
      {gabaritoModalId && (
        <GabaritoRedacaoModal 
          redacaoUploadId={gabaritoModalId} 
