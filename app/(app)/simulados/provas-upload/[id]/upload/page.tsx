...
export default function UploadProvaPage() {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      fd.append('file', file)
      fd.append('itemId', provaId)
      fd.append('itemTipo', 'prova')
  ...
  const handleSave = async (updatedQuestoes?: any[], actionType?: 'enviar_revisao' | 'aprovar', config_estudio?: any) => {
    ...
    try {
      ...
      const { data: dbData } = await (supabase as any).from('provas_upload').select('questoes_json, config_estudio').eq('id', provaId).single()
      ...
      let currentConfig = dbData?.config_estudio || prova?.config_estudio || {}
      ...
      {uploadStep === 'review' && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          <QuestoesEditor 
            ...
            defaultDisciplinaId={prova?.provas_upload_requisicoes?.find((r: any) => r.id_professor === (searchParams.get('prof') || currentUser?.id))?.id_disciplina}
              ...
              {prova.provas_upload_requisicoes.map((req: any, i: number) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(() => {
                      const reqMatchedFile = prova?.config_estudio?.arquivos_originais?.find((a: any) => 
