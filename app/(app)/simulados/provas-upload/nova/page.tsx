...
export default function NovaProvaUploadPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data: provaData, error: provaError } = await (supabase as any)
        .from('provas_upload')
        .insert([payload])
        .select()
        .single()
      ...
      const { error: reqError } = await (supabase as any).from('provas_upload_requisicoes').insert(reqs)
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              <h1 style={{ fontSize: 24, fontWeight: 800, color: 'hsl(var(--text-primary))', margin: 0 }}>Nova Prova via Upload</h1>
          ...
          >
            <Save size={18} /> {loading ? 'Criando...' : 'Criar Prova'}
          </motion.button>
        </div>
        ...
        {successModal && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
              ...
              <h2 style={{ fontSize: 24, fontWeight: 800, color: 'hsl(var(--text-primary))', margin: '0 0 12px' }}>Prova Criada!</h2>
              <p style={{ color: 'hsl(var(--text-secondary))', fontSize: 15, lineHeight: 1.5, margin: '0 0 32px' }}>
                A prova foi criada com sucesso. Os professores vinculados já podem acessar e enviar as questões.
              </p>
