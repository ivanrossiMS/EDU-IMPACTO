...
export default function UploadProvasGerenciamentoPage() {
  ...
  const handleAdaptar = async (prova: any) => {
    ...
    try {
      ...
      if (prova.config_estudio) {
        payload.config_estudio = prova.config_estudio
      }
      payload.titulo = `${prova.titulo || 'Prova'} ADAPTADO`
      payload.status = (prova.questoes_json && prova.questoes_json.length > 0) ? 'em_revisao' : (prova.status || 'aguardando')
      ...
      if (prova.provas_upload_requisicoes && prova.provas_upload_requisicoes.length > 0) {
        ...
        const reqsPayload = oldReqs.map((r: any) => {
          ...
          const hasQs = Array.isArray(prova.questoes_json) && prova.questoes_json.some((q: any) => isQuestionCountableForRequisicao(q, r, oldReqs))
        ...
        const remappedQuestions = (newProva.questoes_json || []).map((q: any) => {
                                      ...
                                      {sortedProvas.map(prova => {
                                        ...
                                        return reqs.map((req: any, rIdx: number) => {
                                          ...
                                          const reqUploadedCount = Array.isArray(prova.questoes_json)
                                            ? prova.questoes_json.filter((q: any) => isQuestionCountableForRequisicao(q, req, reqs)).length
                                            : 0
                                          ...
                                          const isReqEnviada = (req.enviado_em || req.status === 'enviado' || req.status === 'aprovado' || req.status === 'concluido' || prova.status === 'aprovado' || prova.status === 'publicado' || hasQuestionsUploaded) && (req.status !== 'pendente' || hasQuestionsUploaded)
