...
function getSegmentoInfo(serieName: string) {
  ...
  // 2. Ensino Fundamental I (1º ao 5º Ano)
  if (
  ...
  // 4. Ensino Médio (1ª, 2ª, 3ª Série EM, PV, Terceirão)
  if (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            <AnimatePresence>
              {groupedProvas.map(([serie, items]) => {
                ...
                const dataCriacao = prova.created_at ? new Date(prova.created_at).toLocaleDateString('pt-BR') : null
                const reqsWithEnvio = (prova.provas_upload_requisicoes || []).filter((r: any) => r.enviado_em || r.status === 'enviado' || r.status === 'aprovado' || r.status === 'concluido')
                ...
                if (!dataEnvio && computedStatus !== 'aguardando' && prova.updated_at) {
                  dataEnvio = new Date(prova.updated_at).toLocaleDateString('pt-BR')
                }
                      ...
                      <div style={{ display: 'flex', gap: 16, flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingTop: 2, minWidth: 0, width: '100%' }}>
                          ...
                          <div className="responsive-card-info" style={{ display: 'flex', flexWrap: 'wrap', gap: 24, alignItems: 'center', color: 'hsl(var(--text-secondary))', fontSize: 13, fontWeight: 600 }}>
                            ...
                            {prova.criado_por_nome && (
                              <span style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#3b82f6' }}>
                                <User size={14} /> Criado por: {prova.criado_por_nome}
                              </span>
                            )}
                          </div>
                        </div>
