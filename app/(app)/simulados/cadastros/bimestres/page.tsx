...
export default function BimestresPage() {
  const [data, setRaw, { loading, refresh }] = useSupabaseArray<any>('simulados_bimestres', [])
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: 'linear-gradient(135deg, rgba(244,63,94,0.1), rgba(190,18,60,0.1))', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(244,63,94,0.2)' }}>
            <Layers size={24} color="#f43f5e" />
          </div>
          <div>
            <h1 style={{ fontSize: 28, fontWeight: 800, color: 'hsl(var(--text-primary))', margin: 0, letterSpacing: '-0.02em' }}>Bimestres / Períodos</h1>
            <p style={{ color: 'hsl(var(--text-secondary))', margin: '4px 0 0', fontSize: 14 }}>Gerencie os períodos de aplicação dos simulados</p>
          </div>
        </div>
        <button 
          onClick={() => handleOpen()}
      ...
      <div style={{ background: 'hsl(var(--bg-surface))', border: '1px solid hsl(var(--border-subtle))', borderRadius: 24, padding: 24 }}>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div key={item.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 20, background: 'hsl(var(--bg-app))', borderRadius: 16, border: '1px solid hsl(var(--border-subtle))' }}>
                <div>
                  ...
                  <div style={{ fontSize: 13, color: 'hsl(var(--text-secondary))', marginTop: 4 }}>
                    {item.data_inicio ? `${item.data_inicio} até ${item.data_fim}` : 'Período não definido'} • Status: <span style={{ color: item.status === 'ativo' ? '#10b981' : '#ef4444' }}>{item.status?.toUpperCase() || ''}</span>
                  </div>
                </div>
            ...
            {filtered.length === 0 && <div style={{ textAlign: 'center', padding: 40, color: 'hsl(var(--text-muted))' }}>Nenhum bimestre encontrado.</div>}
          </div>
        )}
      </div>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ position: 'relative', width: '100%', maxWidth: 500, background: 'hsl(var(--bg-surface))', borderRadius: 24, padding: 32, border: '1px solid hsl(var(--border-subtle))' }}>
            <h2 style={{ color: 'hsl(var(--text-primary))', margin: '0 0 24px', fontSize: 20 }}>{editingId ? 'Editar Bimestre' : 'Novo Bimestre'}</h2>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label style={{ display: 'block', color: 'hsl(var(--text-secondary))', fontSize: 13, marginBottom: 8 }}>Nome (Ex: 1º Bimestre)</label>
              ...
              <div style={{ display: 'flex', gap: 16 }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', color: 'hsl(var(--text-secondary))', fontSize: 13, marginBottom: 8 }}>Data Início</label>
                  <input type="date" value={formData.data_inicio} onChange={e => setFormData({...formData, data_inicio: e.target.value})} style={{ width: '100%', padding: 14, borderRadius: 12, background: 'hsl(var(--bg-app))', border: '1px solid hsl(var(--border-subtle))', color: 'hsl(var(--text-primary))', fontSize: 15 }} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', color: 'hsl(var(--text-secondary))', fontSize: 13, marginBottom: 8 }}>Data Fim</label>
                  <input type="date" value={formData.data_fim} onChange={e => setFormData({...formData, data_fim: e.target.value})} style={{ width: '100%', padding: 14, borderRadius: 12, background: 'hsl(var(--bg-app))', border: '1px solid hsl(var(--border-subtle))', color: 'hsl(var(--text-primary))', fontSize: 15 }} />
                </div>
              </div>
              <div>
