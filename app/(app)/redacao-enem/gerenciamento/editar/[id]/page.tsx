...
export default function EditarSimuladoPage({
  ...
  useEffect(() => {
    async function loadData() {
      const { data: bims } = await supabase
      ...
      try {
        const res = await fetch(
          "/api/configuracoes/usuarios?type=colaboradores&limit=1000"
        );
        if (res.ok) {
          ...
          setProfessores(
            data.filter(
              (u: any) => u.perfil === "Professor" && u.status === "ativo"
            )
          );
        }
      } catch (e) {}
      ...
      const { data: prova } = await supabase
        ...
        .single();
      if (prova) {
        setTitulo((prova as any).titulo);
        setDataAplicacao((prova as any).data_aplicacao || "");
        setBimestreId((prova as any).id_bimestre || "");
        setValor((prova as any).valor || "");
        setSeries((prova as any).turmas || []);
      }
      ...
      if (reqs && reqs.length > 0) {
        setRequisicoes(
          reqs.map((r: any) => ({
            ...
            qtdQuestoes: r.quantidade_questoes,
          }))
        );
      } else {
        setRequisicoes([]);
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <Link
              href="/redacao-enem/gerenciamento"
              style={{
                width: 40,
                height: 40,
                borderRadius: "50%",
                ...
                {seriesOptions.map((serie) => (
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 600,
                      cursor: "pointer",
                      background: series.includes(serie)
                        ? "rgba(59,130,246,0.15)"
                        : "rgba(100, 116, 139, 0.1)",
                      border: `1px solid ${
                        series.includes(serie) ? "#3b82f6" : "transparent"
                      }`,
                      color: series.includes(serie)
                        ? "#3b82f6"
        ...
        >
          ...
          >
            <div>
              ...
              >
                Delegue a criação de questões para os professores.
              </p>
            </div>
            <button
