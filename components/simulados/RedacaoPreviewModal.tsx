...
export function RedacaoPreviewModal({ questoes, setQuestoes, prova, config, onClose, onSave, saving, isReadOnly = false, printOnMount }: {
  ...
  const getAllItemIds = () => {
    const ids = [
      prova?.id,
      (prova as any)?.id_redacao,
      (prova as any)?.id_prova,
      (prova as any)?.id_prova_upload,
      (prova as any)?.id_simulado,
      (config as any)?.id
    ].filter(Boolean)
  ...
  const getInitialHeaderLayout = () => {
    ...
    let configEstudio = (prova as any)?.config_estudio
    if (typeof configEstudio === 'string') {
      try {
        configEstudio = JSON.parse(configEstudio)
      } catch (e) {}
    }
    const baseLayout = configEstudio?.header_layout || savedLocal || config?.redacao_enem_header_layout
  ...
  }, [itemId, JSON.stringify((prova as any)?.config_estudio?.header_layout), config?.redacao_enem_header_layout])
  ...
  const handleSaveHeaderLayout = async () => {
    ...
    try {
      ...
      alert('Posições do cabeçalho salvas para esta redação com sucesso!')
    } catch (e: any) {
          ...
          <PaginationEngine
            ...
            onRemoveAlternativa={isReadOnly ? () => {} : (qId, aId) => {
              setLocalQuestoes(prev => prev.map(q => {
                ...
                if ((aIdx === undefined || aIdx === -1) && typeof aId === 'string' && aId.startsWith('alt-preview-')) {
                  aIdx = parseInt(aId.replace('alt-preview-', ''))
                }
                if (aIdx === undefined || aIdx === -1 || isNaN(aIdx)) return q
