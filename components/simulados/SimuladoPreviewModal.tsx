...
export function SimuladoPreviewModal({ questoes, setQuestoes, simulado, config, onClose, onSave, saving, isReadOnly = false, printOnMount, isolatedMode = false }: {
  ...
  const [leftMarginOffset, setLeftMarginOffset] = useState<number>(simulado?.config_estudio?.config_margin_left || 0);
  const [rightMarginOffset, setRightMarginOffset] = useState<number>(simulado?.config_estudio?.config_margin_right || 0);
  const [topMarginOffset, setTopMarginOffset] = useState<number>(simulado?.config_estudio?.config_margin_top || 0);
  const [bottomMarginOffset, setBottomMarginOffset] = useState<number>(simulado?.config_estudio?.config_margin_bottom || 0);
  ...
  const handleToggleQuestion = (qId: string) => {
    ...
    const isNowSelected = !next.has(qId)
    ...
    setLocalQuestoes(prev => prev.map(q => 
      q._internalId === qId ? { ...q, excluida: !isNowSelected } : q
    ))
  }
    ...
    >
      ...
      <div className="no-print sidebar-layout" style={{ width: 380, background: 'white', borderRight: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', boxShadow: '4px 0 24px rgba(0,0,0,0.02)', zIndex: 10, flexShrink: 0 }}>
        <div style={{ padding: '24px 32px', borderBottom: '1px solid #e2e8f0' }}>
          ...
          <p style={{ color: '#64748b', fontSize: 13, lineHeight: 1.5, margin: 0 }}>Molde a simulado visualmente para alunos com necessidades específicas. Remova questões indesejadas e altere o tamanho da fonte global.</p>
        </div>
        ...
        <div style={{ padding: '32px', flex: 1, overflowY: 'auto' }}>
          ...
          <div style={{ marginBottom: 32 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 700, color: '#334155', marginBottom: 16 }}>
              <LayoutList size={16} color="#3b82f6" /> Layout da Simulado
            </label>
        ...
        <div className="print-wrapper" style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <PaginationEngine
            ...
            onEditEnunciadoImage={isReadOnly ? () => {} : (qId, imgIndex, newUrl) => {
              setLocalQuestoes(prev => prev.map((q) => {
                if ((q._internalId || q.id) !== qId) return q
                let newImagens = [...(q.imagens || [])]
                if (!newUrl) {
                  // Actually remove the image instead of setting src to empty
