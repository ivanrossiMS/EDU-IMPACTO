...
import {
  ...
  Loader2, Sparkles, Plus, X, ZoomIn, ZoomOut, CheckCircle, Upload, Edit
} from 'lucide-react'
...
export function QuestoesEditor({ questoes, setQuestoes, showAddQuestao = true, defaultDisciplinaId, defaultProfessorId, readOnly = false }: QuestoesEditorProps) {
  ...
  const removeQuestao = (idx: number) => {
    setQuestoes(prev => prev.filter((_, i) => i !== idx).map((q, i) => ({ ...q, numero: i + 1 })))
  }

  const updateQuestaoDescritiva = (qIdx: number, updates: { tipo_questao?: 'multipla_escolha'|'descritiva', linhas_resposta?: number, estilo_espaco?: 'em_branco'|'pautado' }) => {
    setQuestoes(prev => prev.map((q, i) => {
      if (i !== qIdx) return q
  ...
  const comAlternativas = questoes.filter(q => (q.alternativas || []).length > 0).length
    ...
    <div style={{ width: '100%' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, display: 'flex', gap: 12 }}>
          {[
            { label: 'Questões', value: totalQuestoes, color: '#8b5cf6' },
            { label: 'Com alternativas', value: comAlternativas, color: '#3b82f6' },
            { label: 'Com gabarito', value: comGabarito, color: '#10b981' },
          ].map((s, i) => (
            <div key={i} style={{ padding: '10px 18px', borderRadius: 12, background: `${s.color}11`, border: `1px solid ${s.color}22`, display: 'flex', gap: 8, alignItems: 'center' }}>
              <span style={{ fontSize: 22, fontWeight: 800, color: s.color }}>{s.value}</span>
              <span style={{ fontSize: 12, color: 'hsl(var(--text-secondary))', fontWeight: 600 }}>{s.label}</span>
            </div>
          ))}
        </div>
