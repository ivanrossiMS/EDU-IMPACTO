...
export function ProvaQuestaoFormModal({ provaId, questao, defaultProfessorId, defaultDisciplinaId, tituloContexto, onClose, onSave }: { provaId: string, questao?: any, defaultProfessorId?: string, defaultDisciplinaId?: string, tituloContexto?: string, onClose: () => void, onSave: () => void }) {
  ...
  const [linhasResposta, setLinhasResposta] = useState<number>(5)
  const [estiloEspaco, setEstiloEspaco] = useState<'em_branco' | 'pautado'>('em_branco')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let savedEnunciado = getEnunciadoComImagens()
      
      // Clean meta tags to avoid duplication
      savedEnunciado = savedEnunciado.replace(/<meta name="linhas_resposta" content=".*?">/g, '')
      savedEnunciado = savedEnunciado.replace(/<meta name="estilo_espaco" content=".*?">/g, '')
      savedEnunciado = savedEnunciado.replace(/<meta name="turma" content=".*?">/g, '')
      savedEnunciado = savedEnunciado.replace(/<meta name="gerado_por_ia" content=".*?">/g, '')
      savedEnunciado = savedEnunciado.replace(/<meta name="ia_autor_nome" content=".*?">/g, '')
      savedEnunciado = savedEnunciado.replace(/<meta name="ia_prova_titulo" content=".*?">/g, '')

      if (tipoQuestao === 'descritiva') {
        savedEnunciado += `\n<meta name="linhas_resposta" content="${linhasResposta}">`
        savedEnunciado += `\n<meta name="estilo_espaco" content="${estiloEspaco}">`
      }

      if (turma) savedEnunciado += `\n<meta name="turma" content="${turma}">`
      ...
      if (questao?.id) {
        const { error: errDel } = await supabase.from('provas_alternativas').delete().eq('id_questao', qId)
        if (errDel) throw errDel
      }

      const altsData = tipoQuestao === 'multipla_escolha' ? alternativas.filter(a => a.texto.trim() !== '').map((a, i) => ({
        id_questao: qId,
        letra: ['A','B','C','D','E','F','G','H'][i] || a.letter || a.letra,
        texto: a.texto,
        ...
        <div className="modal-body" style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          <div className="form-side" style={{ padding: '32px', overflowY: 'auto', flex: 1 }}>
            <form id="questao-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'hsl(var(--text-secondary))', marginBottom: 8 }}>Tipo de Questão</label>
                <div style={{ display: 'flex', gap: 16 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14, color: 'hsl(var(--text-primary))' }}>
                    <input 
                      type="radio" 
                      name="tipoQuestao" 
                      value="multipla_escolha" 
                      checked={tipoQuestao === 'multipla_escolha'} 
                      onChange={() => setTipoQuestao('multipla_escolha')} 
                    />
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14, color: 'hsl(var(--text-primary))' }}>
                    <input 
                      ...
                      name="tipoQuestao" 
                      value="descritiva" 
                      checked={tipoQuestao === 'descritiva'} 
                      onChange={() => setTipoQuestao('descritiva')} 
                    />
                    Descritiva (Texto)
                  </label>
                </div>
              ...
              {tipoQuestao === 'descritiva' && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'hsl(var(--text-secondary))', marginBottom: 8 }}>Tamanho do Campo</label>
                    <select 
                      value={linhasResposta} 
                      onChange={e => setLinhasResposta(Number(e.target.value))}
                      style={{ width: '100%', padding: '12px 16px', borderRadius: 12, border: '1px solid hsl(var(--border-subtle))', background: 'hsl(var(--bg-surface))', color: 'hsl(var(--text-primary))' }}
                    >
                      <option value={5}>Pequeno (5 linhas)</option>
                      <option value={10}>Médio (10 linhas)</option>
                      <option value={15}>Grande (15 linhas)</option>
                      <option value={20}>Extra Grande (20 linhas)</option>
                    </select>
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'hsl(var(--text-secondary))', marginBottom: 8 }}>Estilo do Espaço</label>
                    <select 
                      value={estiloEspaco} 
                      onChange={e => setEstiloEspaco(e.target.value as 'em_branco' | 'pautado')}
                      style={{ width: '100%', padding: '12px 16px', borderRadius: 12, border: '1px solid hsl(var(--border-subtle))', background: 'hsl(var(--bg-surface))', color: 'hsl(var(--text-primary))' }}
                    >
                      <option value="em_branco">Em Branco</option>
                      <option value="pautado">Pautado (Com Linhas)</option>
                    </select>
                  </div>
                </div>
              )}
