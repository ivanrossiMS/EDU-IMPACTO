...
interface ProvaGabaritoModalProps {
  provaId: string
  onClose: () => void
}

export function ProvaGabaritoModal({ provaId, onClose }: ProvaGabaritoModalProps) {
  ...
  useEffect(() => {
    async function loadData() {
      // 1. Get prova info
      const { data: sim } = await supabase.from('provas').select('*, simulados_bimestres(nome)').eq('id', provaId).single()
      if (sim) setProva(sim)

      // 2. Get prova requisicoes to know the discipline order
      ...
      const { data: q } = await supabase
        ...
        .select('*, simulados_disciplinas(nome), provas_alternativas(id, letra, eh_correta)')
        .eq('id_prova', provaId)
            ...
            <>
              ...
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#475569', fontSize: 13 }}>
                    <Calendar size={14} /> <span>Aplicação: {prova?.data_aplicacao ? new Date(prova.data_aplicacao).toLocaleDateString('pt-BR') : 'Data não definida'}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#475569', fontSize: 13 }}>
                    <Layers size={14} /> <span>{prova?.simulados_bimestres?.nome || 'Sem Bimestre'}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#475569', fontSize: 13 }}>
                    <Users size={14} /> <span>Turmas: {prova?.turmas?.join(', ') || 'Geral'}</span>
                  </div>
                    ...
                    <div key={disciplina} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: 16, breakInside: 'avoid' }}>
                      ...
                      <div style={{ columnCount: 2, columnGap: 16 }}>
                        {questoesDisciplina.map((q) => {
                          ...
                          const alternativaCorreta = q.provas_alternativas?.find((a: any) => a.eh_correta || a.correct)
              ...
              {questoes.length === 0 && !loading && (
                <div style={{ textAlign: 'center', color: 'hsl(var(--text-secondary))', padding: 60, border: '1px dashed hsl(var(--border-subtle))', borderRadius: 20 }}>
                  <p>Este prova ainda não possui questões cadastradas.</p>
                </div>
              )}
            </>
          )}
