...
import { MessageCircle, X, Search, Send, Plus, Users, User, ArrowLeft, CheckCheck, ChevronDown, ChevronRight, Mail, Reply, Paperclip, Smile, Shield, GraduationCap, DollarSign, FileText, Building, Phone, UserCheck } from 'lucide-react'
...
export default function FloatingChat() {
  ...
  const [floatingTab, setFloatingTab] = useState<'familias' | 'equipes'>('familias')
  ...
  const unreadCount = (myChats.reduce((acc, c) => {
    ...
    return acc
  }, 0)) + ((myGroups).reduce((acc, eq: any) => {
    const msgs = messages[`equipe-${eq.id}`] || []
    if (msgs.length === 0) return acc
    const lastMsg = msgs[msgs.length - 1]
    const isOther = String(lastMsg.authorId || '') !== String(currentUser?.id || '')
    // Se a última mensagem for de outro, incrementa 1 para o canal (como um indicador de canal com novidade)
    if (isOther) {
      return acc + 1
    }
    return acc
  ...
  // ── Equipe icon helper
  const getEquipeIconFC = (icone: string, cor: string, size = 16) => {
    const props = { size, color: cor }
    const map: Record<string, React.ReactNode> = {
      Shield: <Shield {...props} />, GraduationCap: <GraduationCap {...props} />,
      FileText: <FileText {...props} />, DollarSign: <DollarSign {...props} />,
      Phone: <Phone {...props} />, Building: <Building {...props} />,
      Users: <Users {...props} />, UserCheck: <UserCheck {...props} />,
    }
    return map[icone] || <Users {...props} />
  }

  const activeEquipe = (equipes || []).find((eq: any) => eq?.id === activeEquipeId)
  const equipeMessages = activeEquipeId ? messages[`equipe-${activeEquipeId}`] || [] : []

  const handleEquipeSendFloating = (e: React.FormEvent) => {
    e.preventDefault()
    if (!equipeInputText.trim() || !activeEquipeId || !activeEquipe) return
    const chatKey = `equipe-${activeEquipeId}`
    const newMsg = {
      id: Date.now(), text: equipeInputText, sender: 'us' as const,
    ...
    setChatsList(prev => {
      ...
      if (existing) return [{ ...existing, preview: equipeInputText, time: newMsg.time, date: newMsg.date }, ...prev.filter(c => String(c.id) !== chatKey)]
      return [{ id: chatKey, name: activeEquipe.nome, status: 'online', preview: equipeInputText, time: newMsg.time, date: newMsg.date, unread: 0, tag: activeEquipe.nome }, ...prev]
    })
    setEquipeInputText('')
  }
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setChatsList(prev => prev.map(c => String(c.id) === String(activeChat.id) ? { 
      ...
      time: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
      date: new Date().toLocaleDateString('pt-BR'),
      unread: (prev.find(ch => String(ch.id) === String(activeChat.id))?.unread || 0) + 1
    } : c))
    setInputText('')
      ...
      if (currentHourStr < inicio || currentHourStr > fim) {
        ...
        setTimeout(() => {
          ...
          setChatsList(prev => {
            ...
            return prev.map(c => String(c.id) === String(chatId) ? { 
              ...c, 
    ...
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div style={{ padding: '16px 20px', background: 'linear-gradient(135deg, #1e1b4b, #4338ca)', color: 'white' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: !isAlunoOrResponsavel ? 12 : 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  {(activeChat || showNewChat || activeEquipeId) ? (
                    <button onClick={() => { setActiveChat(null); setShowNewChat(false); setActiveEquipeId(null); }} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', width: 32, height: 32, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                      <ArrowLeft size={18} />
                    </button>
                  ) : (
                    <div style={{ width: 36, height: 36, background: 'rgba(255,255,255,0.2)', borderRadius: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {floatingTab === 'equipes' ? <Shield size={18} /> : <Mail size={18} />}
                    </div>
                  )}
                  <div>
                    <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>
                      {activeEquipeId && activeEquipe ? activeEquipe.nome
                        : activeChat ? (activeChat.tag || 'Conversa sem assunto')
                        : showNewChat ? 'Nova Mensagem Oficial'
                        : floatingTab === 'equipes' ? 'Canais das Equipes'
                        : 'Caixa de Entrada'}
                    </h3>
                    <div style={{ fontSize: 11, opacity: 0.8 }}>
                      {activeChat ? (
                        <span style={{ padding: '2px 8px', borderRadius: 6, background: 'rgba(255,255,255,0.2)', fontWeight: 600 }}>{activeChat.name}</span>
                      ) : activeEquipeId && activeEquipe ? (
                        <span style={{ opacity: 0.75 }}>Canal interno da equipe</span>
              ...
              {/* Tab switcher — only for staff/collaborators */}
              {!isAlunoOrResponsavel && !activeChat && !showNewChat && !activeEquipeId && (
                <div style={{ display: 'flex', background: 'rgba(255,255,255,0.1)', borderRadius: 10, padding: 3, gap: 2 }}>
                  {[{id:'familias',label:'Famílias',icon:<Mail size={11}/>},{id:'equipes',label:'Equipes',icon:<Shield size={11}/>}].map(t => (
                    <button key={t.id} onClick={() => setFloatingTab(t.id as any)} style={{
                      flex:1, padding:'5px 8px', borderRadius:8, border:'none', cursor:'pointer', fontSize:11, fontWeight:700,
                      display:'flex', alignItems:'center', justifyContent:'center', gap:4, transition:'all 0.2s',
                      background: floatingTab===t.id ? 'rgba(255,255,255,0.9)' : 'transparent',
                      color: floatingTab===t.id ? '#4338ca' : 'rgba(255,255,255,0.8)',
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              {!activeChat && !showNewChat && !activeEquipeId && (
                <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                  ...
                  {/* ── Equipes list */}
                  {floatingTab === 'equipes' && !isAlunoOrResponsavel && (
                    <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
                      {(equipes || []).length === 0 ? (
                        <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8' }}>
                          <Shield size={32} style={{ opacity: 0.2, marginBottom: 12 }} />
                          <p style={{ fontSize: 13 }}>Nenhuma equipe criada ainda.</p>
                        </div>
                      ) : (
                        (equipes || []).map((eq: any) => {
                          const msgs = messages[`equipe-${eq.id}`] || []
                          const lastMsg = msgs[msgs.length - 1]
                          return (
                            <div key={eq.id} onClick={() => setActiveEquipeId(eq.id)}
                              style={{ display:'flex', gap:10, padding:'12px 10px', borderRadius:14, cursor:'pointer', transition:'background 0.2s', marginBottom:4 }}
                              onMouseEnter={e => e.currentTarget.style.background='rgba(0,0,0,0.03)'}
                              onMouseLeave={e => e.currentTarget.style.background='transparent'}
                            >
                              <div style={{ width:42, height:42, borderRadius:12, background:eq.cor||'#6366f1', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                                {getEquipeIconFC(eq.icone, 'white', 18)}
                              </div>
                              <div style={{ flex:1, minWidth:0 }}>
                                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}>
                                  <span style={{ fontWeight:700, fontSize:14, color:'#1e293b' }}>{eq.nome}</span>
                                  {lastMsg && <span style={{ fontSize:11, color:'#94a3b8' }}>{lastMsg.time}</span>}
                                </div>
                                <div style={{ fontSize:12, color:'#64748b', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                                  {lastMsg ? <><strong>{lastMsg.author?.split(' ')[0]}:</strong> {lastMsg.text}</> : <em>Nenhuma mensagem ainda</em>}
                                </div>
                              </div>
                            </div>
                          )
                  ...
                  {/* ── Famílias list */}
                  {(floatingTab === 'familias' || isAlunoOrResponsavel) && (
              ...
              {/* ── Equipe group chat panel */}
              {activeEquipeId && activeEquipe && (
                <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                  <div style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {equipeMessages.length === 0 && (
                      <div style={{ textAlign: 'center', padding: '40px 20px', color: '#94a3b8' }}>
                        <div style={{ width:48,height:48,borderRadius:24,background:`${activeEquipe.cor||'#6366f1'}18`,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 12px' }}>
                          {getEquipeIconFC(activeEquipe.icone, activeEquipe.cor||'#6366f1', 22)}
                        </div>
                        <p style={{ fontSize: 13 }}>Envie a primeira mensagem para o canal!</p>
                      </div>
                    )}
                    {equipeMessages.map((msg: any) => {
                      const isMe = String(msg.authorId||'') === String(currentUser?.id||'') || msg.sender === 'us'
                      return (
                        <div key={msg.id} style={{ display:'flex', flexDirection:'column', alignItems: isMe ? 'flex-end':'flex-start', marginBottom: 4 }}>
                          <div style={{
                            background: isMe ? (activeEquipe.cor||'#6366f1') : 'white',
                            ...
                            boxShadow: isMe ? `0 4px 12px ${activeEquipe.cor||'#6366f1'}30` : '0 2px 8px rgba(0,0,0,0.03)'
                          }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6, paddingBottom: 6, borderBottom: isMe ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(0,0,0,0.05)', gap: 12 }}>
                               <div style={{ fontSize: 11, fontWeight: 700, opacity: 0.9 }}>{msg.author || (isMe ? 'Você' : 'Usuário')}</div>
                               <div style={{ fontSize: 10, opacity: 0.7, whiteSpace: 'nowrap' }}>{msg.date} às {msg.time}</div>
                            </div>
                  ...
                  <div style={{ padding:'12px 16px', background:'white', borderTop:'1px solid rgba(0,0,0,0.05)' }}>
                    <form onSubmit={handleEquipeSendFloating} style={{ display:'flex', gap:8 }}>
                      <input value={equipeInputText} onChange={e => setEquipeInputText(e.target.value)}
                        placeholder={`Mensagem para ${activeEquipe.nome}...`}
                        style={{ flex:1, padding:'10px 14px', borderRadius:20, border:'1px solid #e2e8f0', fontSize:13, outline:'none' }}
                      />
                      <button type="submit" disabled={!equipeInputText.trim()}
                        style={{ width:40, height:40, borderRadius:20, border:'none', display:'flex', alignItems:'center', justifyContent:'center', cursor: equipeInputText.trim() ? 'pointer':'default', background: equipeInputText.trim() ? (activeEquipe.cor||'#6366f1') : '#e2e8f0', color: equipeInputText.trim() ? 'white' : '#94a3b8', transition:'all 0.2s' }}>
                        <Send size={16} />
                      </button>
                    </form>
                  </div>
