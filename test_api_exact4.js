...
async function check() {
  ...
  let query = supabase.from('comunicados').select('id, titulo, destino, dados').not('id', 'like', 'AD-COM-REL-COLAB-%');
  ...
  if (data) {
    data.forEach(c => console.log(c.id, '|', c.titulo, '| destino:', c.destino, '| tipo:', c.dados?.tipo));
  }
}
check();
